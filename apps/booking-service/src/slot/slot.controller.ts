...
import { Roles } from '@app/common/decorator/role.decorator';
import { UserRole } from '@app/common/database/schemas/common.enums';
import { ApiResponse as AppApiResponse } from '@app/common/response/api-response'; // ← added
...
export class SlotController {
  ...
  async getAvailableSlots(
    @Query() query: GetAvailableSlotsDto,
    @Headers('accept-language') acceptLanguage?: string, // ← added
  ) {
    const lang = resolveLang(acceptLanguage); // ← added
    const data: GroupedAvailableSlotsDto =
      await this.slotManagementService.getAvailableSlots(query);
    return AppApiResponse.success({ lang, messageKey: 'slot.LIST', data }); // ← added
  }
}
