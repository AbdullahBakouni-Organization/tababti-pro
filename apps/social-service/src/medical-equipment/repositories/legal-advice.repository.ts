...
import { EntityRequestStatus, UserRole, LegalAdviceCategory } from '@app/common/database/schemas/common.enums';
...
export class LegalAdviceRepository {
    ...
    async findById(requestId: string): Promise<LegalAdviceRequest> {
        ...
        const request = await this.legalAdviceModel.findById(
    ...
    async updateStatus(
        ...
        const request = await this.legalAdviceModel.findByIdAndUpdate(
            new Types.ObjectId(requestId),
            updateData,
            { new: true },
        ).lean();
