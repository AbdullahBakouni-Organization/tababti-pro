...
// ============================================
// Doctor Bookings Query Service (v2) — with i18n
// ============================================
...
export interface RescheduleResult extends ServiceResult {
  bookingId: string;
  slotId: string;
}
...
export class DoctorBookingsQueryService {
  ...
  // Returns raw GetDoctorBookingsResponseDto — controller wraps it
  // ============================================
  ...
  async getDoctorBookings(
    ...
    this.logger.log(`Getting bookings for doctor ${doctorId}`);
    ...
    const cacheKey = `doctor:bookings:${doctorId}:${JSON.stringify(dto)}`;
    const cached =
      await this.cacheService.get<GetDoctorBookingsResponseDto>(cacheKey);
    if (cached) {
      this.logger.debug(`Cache hit for bookings: doctor ${doctorId}`);
      return cached;
    }
    ...
    if (date) {
      ...
      matchConditions.bookingDate = { $gte: selectedDate, $lt: nextDay };
    } else if (startDate || endDate) {
      matchConditions.bookingDate = {};
      if (startDate) {
        const start = new Date(startDate);
        start.setHours(0, 0, 0, 0);
        matchConditions.bookingDate.$gte = start;
      }
      if (endDate) {
        const end = new Date(endDate);
        end.setHours(23, 59, 59, 999);
        matchConditions.bookingDate.$lte = end;
      }
    }
    ...
    if (locationEntityName) {
      matchConditions['location.entity_name'] = {
        $regex: locationEntityName,
        $options: 'i',
      };
    }

    if (locationType) {
      matchConditions['location.type'] = locationType;
    }
          ...
          totals: [
            {
              $group: {
                _id: null,
                totalRevenue: { $sum: '$price' },
                avgDuration: { $avg: '$inspectionDuration' },
              },
            },
          ],
        },
    ...
    for (const group of result.byStatus || []) {
      byStatus[group._id] = group.count;
    }

    const averageDuration = result.totals[0]?.avgDuration
      ? Math.round(result.totals[0].avgDuration)
      : 0;
    ...
    await this.cacheService.set(cacheKey, response, this.CACHE_TTL);
    this.logger.log(
      `Bookings retrieved for doctor ${doctorId}: ${totalItems} total`,
    );
  ...
  async rescheduleBooking(
    ...
    this.logger.log(`Doctor ${doctorId} rescheduling booking ${dto.bookingId}`);
    ...
    // Ownership is already enforced by the query above, but belt-and-suspenders:
    if (booking.doctorId.toString() !== doctorId) {
      throw new ForbiddenException('booking.ACCESS_DENIED');
    }
    ...
    this.logger.log(
      `Booking ${dto.bookingId} marked RESCHEDULED. Slot ${slot._id.toString()} freed.`,
    );

    // Publish Kafka event — notification service handles FCM push to patient
    this.publishRescheduleNotification(doctorId, booking, slot);

    return {
      messageKey: 'booking.RESCHEDULED',
      bookingId: booking._id.toString(),
      slotId: slot._id.toString(),
    };
  }
  ...
  private publishRescheduleNotification(
    doctorId: string,
    booking: BookingDocument,
    ...
    const event = {
      ...
      metadata: { source: 'doctor-service-v2', version: '1.0' },
    };
    ...
    try {
      ...
      this.logger.log(
        `Reschedule notification published for booking ${booking._id!.toString()}`,
      );
    } catch (error) {
      const err = error as Error;
      this.logger.error(
        `Failed to publish reschedule notification: ${err.message}`,
        err.stack,
      );
    }
  }
