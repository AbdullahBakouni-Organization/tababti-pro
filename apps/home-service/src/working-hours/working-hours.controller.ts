...
export class WorkingHoursController {
  ...
  // ==========================================================================
  // Add working hours (initial setup)
  // ==========================================================================
  ...
  @ApiResponse({
    status: HttpStatus.CREATED,
    description: 'Working hours added successfully and slots generation initiated',
    type: WorkingHoursResponseDto,
  })
  @ApiResponse({
    status: HttpStatus.BAD_REQUEST,
  ...
  async addWorkingHours(
    ...
    const { messageKey, ...data } = await this.workingHoursService.addWorkingHours(
      doctorId,
      addWorkingHoursDto,
    );
  ...
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Invalid doctor ID format' })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Doctor not found' })
  async getWorkingHours(@Req() req: any) {
    const doctorId = new ParseMongoIdPipe().transform(
      req.user.entity._id.toString(),
    ...
    const { messageKey, ...data } =
      await this.workingHoursService.getWorkingHours(doctorId);
    return AppResponse.success({ lang: getLang(), messageKey, data });
  }
  ...
  async checkConflicts(
    ...
    const data = await this.workingHoursService.checkWorkingHoursConflicts(
      doctorId,
      updateDto,
    );
  ...
  // ==========================================================================
  // Update working hours (confirmed)
  // ==========================================================================
  ...
  @ApiResponse({ status: 409, description: 'Conflicts exist but not confirmed' })
  async updateWorkingHours(
    @Body() updateDto: UpdateWorkingHoursDto,
    @Req() req: any,
  ) {
    ...
    const { messageKey } = await this.workingHoursService.updateWorkingHours(
      doctorId,
      updateDto,
    );
