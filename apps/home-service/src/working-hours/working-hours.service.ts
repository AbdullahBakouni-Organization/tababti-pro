...
export interface AddWorkingHoursResult {
  messageKey: string;
  doctorId: string;
  workingHours: any[];
  slotsGenerated: number;
...
export interface GetWorkingHoursResult {
  messageKey: string;
  doctorId: string;
  workingHours: any[];
  inspectionDuration: number;
...
export class WorkingHoursService {
  ...
  async addWorkingHours(
    ...
    return {
      // First-time setup → slots are being generated, otherwise a plain update
      messageKey: isFirstTime
        ? 'working_hours.SLOTS_GENERATING'
  ...
  async checkWorkingHoursConflicts(
    ...
    const response: ConflictCheckResponseDto = {
      ...
      // warningMessage carries the i18n key — controller resolves it via getLang().
      // The counts needed to build a localised string are already in summary above.
      warningMessage: hasConflicts
        ? 'working_hours.CONFLICT_WARNING'
        : undefined,
    };
  ...
  async updateWorkingHours(
    ...
    return { messageKey: 'working_hours.UPDATED' };
  }
  ...
  async getWorkingHours(doctorId: string): Promise<GetWorkingHoursResult> {
    if (!Types.ObjectId.isValid(doctorId)) {
      throw new BadRequestException('doctor.INVALID_ID');
    }

    const cacheKey = `doctor:${doctorId}:working-hours`;
    const cached = await this.cacheManager.get<GetWorkingHoursResult>(cacheKey);
    ...
    const result: GetWorkingHoursResult = {
      messageKey: 'working_hours.FETCHED',
      doctorId,
      workingHours: doctor.workingHours || [],
      inspectionDuration: doctor.inspectionDuration,
      inspectionPrice: doctor.inspectionPrice,
  ...
  private validateWorkingHours(workingHours: WorkingHour[]): void {
    ...
    for (const wh of workingHours) {
      if (!this.isValidTimeRange(wh.startTime, wh.endTime)) {
        throw new BadRequestException('working_hours.INVALID_TIME_RANGE');
      }
      ...
      for (const existingWh of existing) {
        ...
        if (sameLocation) {
          throw new BadRequestException('working_hours.DUPLICATE_LOCATION');
        }
        ...
        if (
          ...
          throw new BadRequestException('working_hours.INTERNAL_OVERLAP');
        }
      }
  ...
  private checkIfSameAsExisting(
    ...
    for (const newWh of newHours) {
      ...
      if (match) {
        throw new BadRequestException('working_hours.SAME_AS_EXISTING');
      }
    }
  }
