...
export class BookingSeeder {
    async seed() {
        ...
        for (let i = 0; i < 100; i++) {
            // Pick a random doctor and user
            const doctor = doctors[Math.floor(Math.random() * doctors.length)];
            const user = users[Math.floor(Math.random() * users.length)];

            // Random booking date within the next 30 days
            const bookingDate = faker.date.soon({ days: 30 });

            // Random time slot 9:00-16:00, 30 min increments
            ...
            // Random location (Clinic or Hospital)
            const location = {
            ...
            const booking = await bookingModel.create({
                userId: user._id,
                doctorId: doctor._id,
                slotId: new Types.ObjectId(), // Random ObjectId, replace with real slot if available
            ...
            console.log(`✅ Booking ${createdCount + 1} created for doctor ${doctor.firstName}`);
            createdCount++;
        }
